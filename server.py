#!/usr/bin/env python3

import asyncio
import websockets
import json
import random
import sys

# The bind address is configurable because the multiplayer server and the
# reverse proxy run in separate containers: bound to localhost it is
# unreachable from nginx and /p2 returns 502. config.py is optional here,
# unlike in app.py, so that server.py still runs standalone.
try:
    import config
except ModuleNotFoundError:
    config = None

BIND = getattr(config, "MULTIPLAYER_BIND", None) or "localhost"

server_status = {
    "waiting": {},
    "users": [],
    "invites": {}
}
consonants = "bcdfghjklmnpqrstvwxyz"

def msgobj(msg_type, value=None):
    if value == None:
        return json.dumps({"type": msg_type})
    else:
        return json.dumps({"type": msg_type, "value": value})

def status_event():
    value = []
    for id, userDiff in server_status["waiting"].items():
        value.append({
            "id": id,
            "diff": userDiff["diff"]
        })
    return msgobj("users", value)

def get_invite():
    return "".join([random.choice(consonants) for x in range(5)])

def get_message_dict(message):
    try:
        data = json.loads(message)
    except (TypeError, UnicodeDecodeError, json.decoder.JSONDecodeError):
        return {}
    return data if type(data) is dict else {}

async def wait_all(awaitables):
    if awaitables:
        await asyncio.gather(*awaitables, return_exceptions=True)

async def notify_status():
    ready_users = [user for user in server_status["users"] if "ws" in user and user["action"] == "ready"]
    if ready_users:
        sent_msg = status_event()
        await wait_all([user["ws"].send(sent_msg) for user in ready_users])

async def connection(ws, path):
    # User connected
    user = {
        "ws": ws,
        "action": "ready",
        "session": False,
        "name": None,
        "don": None
    }
    server_status["users"].append(user)
    try:
        # Notify user about other users
        await ws.send(status_event())
        while True:
            try:
                message = await asyncio.wait_for(ws.recv(), timeout=10)
            except asyncio.TimeoutError:
                # Keep user connected
                pong_waiter = await ws.ping()
                try:
                    await asyncio.wait_for(pong_waiter, timeout=10)
                except asyncio.TimeoutError:
                    # Disconnect
                    break
            except websockets.exceptions.ConnectionClosed:
                # Connection closed
                break
            else:
                # Message received
                data = get_message_dict(message)
                action = user["action"]
                msg_type = data["type"] if "type" in data else None
                value = data["value"] if "value" in data else None
                if action == "ready":
                    # Not playing or waiting
                    if msg_type == "join":
                        if type(value) is not dict:
                            continue
                        waiting = server_status["waiting"]
                        id = value.get("id")
                        diff = value.get("diff")
                        user["name"] = value.get("name")
                        user["don"] = value.get("don")
                        user["mods"] = value.get("mods")
                        user["soundEffect"] = value.get("soundEffect")
                        if not id or not diff:
                            continue
                        if id not in waiting:
                            # Wait for another user
                            user["action"] = "waiting"
                            user["gameid"] = id
                            waiting[id] = {
                                "user": user,
                                "diff": diff
                            }
                            await ws.send(msgobj("waiting"))
                        else:
                            # Join the other user and start game
                            user["name"] = value.get("name")
                            user["don"] = value.get("don")
                            user["other_user"] = waiting[id]["user"]
                            waiting_diff = waiting[id]["diff"]
                            del waiting[id]
                            if "ws" in user["other_user"]:
                                user["action"] = "loading"
                                user["other_user"]["action"] = "loading"
                                user["other_user"]["other_user"] = user
                                user["other_user"]["player"] = 1
                                user["player"] = 2
                                await wait_all([
                                    ws.send(msgobj("gameload", {
                                        "diff": waiting_diff,
                                        "player": 2,
                                        "mods": user["other_user"].get("mods"),
                                        "soundEffect": user["other_user"].get("soundEffect")
                                    })),
                                    user["other_user"]["ws"].send(msgobj("gameload", {
                                        "diff": diff,
                                        "player": 1,
                                        "mods": user.get("mods"),
                                        "soundEffect": user.get("soundEffect")
                                    })),
                                    ws.send(msgobj("name", {
                                        "name": user["other_user"]["name"],
                                        "don": user["other_user"]["don"]
                                    })),
                                    user["other_user"]["ws"].send(msgobj("name", {
                                        "name": user["name"],
                                        "don": user["don"]
                                    }))
                                ])
                            else:
                                # Wait for another user
                                user.pop("other_user", None)
                                user["action"] = "waiting"
                                user["gameid"] = id
                                waiting[id] = {
                                    "user": user,
                                    "diff": diff
                                }
                                await ws.send(msgobj("waiting"))
                        # Update others on waiting players
                        await notify_status()
                    elif msg_type == "invite":
                        if type(value) is not dict:
                            await ws.send(msgobj("gameend"))
                        elif "id" in value and value["id"] == None:
                            # Session invite link requested
                            invite = get_invite()
                            server_status["invites"][invite] = user
                            user["action"] = "invite"
                            user["session"] = invite
                            user["name"] = value.get("name")
                            user["don"] = value.get("don")
                            await ws.send(msgobj("invite", invite))
                        elif "id" in value and value["id"] in server_status["invites"]:
                            # Join a session with the other user
                            user["name"] = value.get("name")
                            user["don"] = value.get("don")
                            user["other_user"] = server_status["invites"][value["id"]]
                            del server_status["invites"][value["id"]]
                            if "ws" in user["other_user"]:
                                user["other_user"]["other_user"] = user
                                user["action"] = "invite"
                                user["session"] = value["id"]
                                user["other_user"]["player"] = 1
                                user["player"] = 2
                                await wait_all([
                                    ws.send(msgobj("session", {"player": 2})),
                                    user["other_user"]["ws"].send(msgobj("session", {"player": 1})),
                                    ws.send(msgobj("invite")),
                                    ws.send(msgobj("name", {
                                        "name": user["other_user"]["name"],
                                        "don": user["other_user"]["don"]
                                    })),
                                    user["other_user"]["ws"].send(msgobj("name", {
                                        "name": user["name"],
                                        "don": user["don"]
                                    }))
                                ])
                            else:
                                user.pop("other_user", None)
                                await ws.send(msgobj("gameend"))
                        else:
                            # Session code is invalid
                            await ws.send(msgobj("gameend"))
                elif action == "waiting" or action == "loading" or action == "loaded":
                    # Waiting for another user
                    if msg_type == "leave":
                        # Stop waiting
                        if user["session"]:
                            if "other_user" in user and "ws" in user["other_user"]:
                                user["action"] = "songsel"
                                await wait_all([
                                    ws.send(msgobj("left")),
                                    user["other_user"]["ws"].send(msgobj("users", []))
                                ])
                            else:
                                user["action"] = "ready"
                                user["session"] = False
                                await wait_all([
                                    ws.send(msgobj("gameend")),
                                    ws.send(status_event())
                                ])
                        else:
                            server_status["waiting"].pop(user.get("gameid"), None)
                            user.pop("gameid", None)
                            user["action"] = "ready"
                            await wait_all([
                                ws.send(msgobj("left")),
                                notify_status()
                            ])
                    if action == "loading" and "other_user" in user and "ws" in user["other_user"]:
                        if msg_type == "gamestart":
                            user["action"] = "loaded"
                            if user["other_user"]["action"] == "loaded":
                                user["action"] = "playing"
                                user["other_user"]["action"] = "playing"
                                sent_msg = msgobj("gamestart")
                                await wait_all([
                                    ws.send(sent_msg),
                                    user["other_user"]["ws"].send(sent_msg)
                                ])
                elif action == "playing":
                    # Playing with another user
                    if "other_user" in user and "ws" in user["other_user"]:
                        if msg_type == "note"\
                            or msg_type == "drumroll"\
                            or msg_type == "branch"\
                            or msg_type == "gameresults":
                            await user["other_user"]["ws"].send(msgobj(msg_type, value))
                        elif msg_type == "songsel" and user["session"]:
                            user["action"] = "songsel"
                            user["other_user"]["action"] = "songsel"
                            sent_msg1 = msgobj("songsel")
                            sent_msg2 = msgobj("users", [])
                            await wait_all([
                                ws.send(sent_msg1),
                                ws.send(sent_msg2),
                                user["other_user"]["ws"].send(sent_msg1),
                                user["other_user"]["ws"].send(sent_msg2)
                            ])
                        elif msg_type == "gameend":
                            # User wants to disconnect
                            user["action"] = "ready"
                            user["other_user"]["action"] = "ready"
                            sent_msg1 = msgobj("gameend")
                            sent_msg2 = status_event()
                            await wait_all([
                                ws.send(sent_msg1),
                                ws.send(sent_msg2),
                                user["other_user"]["ws"].send(sent_msg1),
                                user["other_user"]["ws"].send(sent_msg2)
                            ])
                            user["other_user"].pop("other_user", None)
                            user.pop("other_user", None)
                    else:
                        # Other user disconnected
                        user["action"] = "ready"
                        user["session"] = False
                        await wait_all([
                            ws.send(msgobj("gameend")),
                            ws.send(status_event())
                        ])
                elif action == "invite":
                    if msg_type == "leave":
                        # Cancel session invite
                        server_status["invites"].pop(user["session"], None)
                        user["action"] = "ready"
                        user["session"] = False
                        if "other_user" in user and "ws" in user["other_user"]:
                            user["other_user"]["action"] = "ready"
                            user["other_user"]["session"] = False
                            sent_msg = status_event()
                            await wait_all([
                                ws.send(msgobj("left")),
                                ws.send(sent_msg),
                                user["other_user"]["ws"].send(msgobj("gameend")),
                                user["other_user"]["ws"].send(sent_msg)
                            ])
                        else:
                            await wait_all([
                                ws.send(msgobj("left")),
                                ws.send(status_event())
                            ])
                    elif msg_type == "songsel" and "other_user" in user:
                        if "ws" in user["other_user"]:
                            user["action"] = "songsel"
                            user["other_user"]["action"] = "songsel"
                            sent_msg = msgobj(msg_type)
                            await wait_all([
                                ws.send(sent_msg),
                                user["other_user"]["ws"].send(sent_msg)
                            ])
                        else:
                            user["action"] = "ready"
                            user["session"] = False
                            await wait_all([
                                ws.send(msgobj("gameend")),
                                ws.send(status_event())
                            ])
                elif action == "songsel":
                    # Session song selection
                    if "other_user" in user and "ws" in user["other_user"]:
                        if msg_type in ("songsel", "catjump", "folder", "folderup", "jumpto"):
                            # Change song select position. All of them are
                            # echoed to both sides for the same reason: the
                            # two clients apply the move together rather
                            # than one leading and the other following.
                            # jumpto names a song by its folder path, which
                            # is what random and search need -- neither is
                            # a move the peer could work out for itself.
                            if user["other_user"]["action"] == "songsel" and type(value) is dict:
                                value["player"] = user["player"]
                                sent_msg = msgobj(msg_type, value)
                                await wait_all([
                                    ws.send(sent_msg),
                                    user["other_user"]["ws"].send(sent_msg)
                                ])
                        elif msg_type == "crowns" or msg_type == "getcrowns":
                            if user["other_user"]["action"] == "songsel":
                                sent_msg = msgobj(msg_type, value)
                                await wait_all([
                                    user["other_user"]["ws"].send(sent_msg)
                                ])
                        elif msg_type == "join":
                            # Start game
                            if type(value) is not dict:
                                continue
                            id = value.get("id")
                            diff = value.get("diff")
                            mods = value.get("mods")
                            soundEffect = value.get("soundEffect")
                            if not id or not diff:
                                continue
                            if user["other_user"]["action"] == "waiting":
                                user["action"] = "loading"
                                user["other_user"]["action"] = "loading"
                                await wait_all([
                                    ws.send(msgobj("gameload", {
                                        "diff": user["other_user"]["gamediff"],
                                        "mods": user["other_user"].get("gamemods"),
                                        "soundEffect": user["other_user"].get("gameSoundEffect")
                                    })),
                                    user["other_user"]["ws"].send(msgobj("gameload", {
                                        "diff": diff,
                                        "mods": mods,
                                        "soundEffect": soundEffect
                                    }))
                                ])
                            else:
                                user["action"] = "waiting"
                                user["gamediff"] = diff
                                user["gamemods"] = mods
                                user["gameSoundEffect"] = soundEffect
                                await user["other_user"]["ws"].send(msgobj("users", [{
                                    "id": id,
                                    "diff": diff
                                }]))
                        elif msg_type == "gameend":
                            # User wants to disconnect
                            user["action"] = "ready"
                            user["session"] = False
                            user["other_user"]["action"] = "ready"
                            user["other_user"]["session"] = False
                            sent_msg1 = msgobj("gameend")
                            sent_msg2 = status_event()
                            await wait_all([
                                ws.send(sent_msg1),
                                user["other_user"]["ws"].send(sent_msg1)
                            ])
                            await wait_all([
                                ws.send(sent_msg2),
                                user["other_user"]["ws"].send(sent_msg2)
                            ])
                            user["other_user"].pop("other_user", None)
                            user.pop("other_user", None)
                    else:
                        # Other user disconnected
                        user["action"] = "ready"
                        user["session"] = False
                        await wait_all([
                            ws.send(msgobj("gameend")),
                            ws.send(status_event())
                        ])
    finally:
        # User disconnected
        user.pop("ws", None)
        if user in server_status["users"]:
            server_status["users"].remove(user)
        if "other_user" in user and "ws" in user["other_user"]:
            user["other_user"]["action"] = "ready"
            user["other_user"]["session"] = False
            await wait_all([
                user["other_user"]["ws"].send(msgobj("gameend")),
                user["other_user"]["ws"].send(status_event())
            ])
            user["other_user"].pop("other_user", None)
        if user["action"] == "waiting":
            server_status["waiting"].pop(user.get("gameid"), None)
            await notify_status()
        elif user["action"] == "invite":
            server_status["invites"].pop(user["session"], None)

port = int(sys.argv[1]) if len(sys.argv) > 1 else 34802
print('Starting server on %s port %d' % (BIND, port))

async def main():
    async with websockets.serve(connection, BIND, port):
        await asyncio.Future()

try:
    asyncio.run(main())
except KeyboardInterrupt:
    print("Stopping server")
